...
def delete_and_recreate_topic(
    ...
    for topic, future in fs.items():
        try:
            future.result()
            print(f"✅ Topic '{topic}' recreated successfully with {num_partitions} partitions!")
        except Exception as e:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python clear_kafka_topic.py <topic_name> [bootstrap_server]")
        print("Example: python clear_kafka_topic.py admissions-inbound")
